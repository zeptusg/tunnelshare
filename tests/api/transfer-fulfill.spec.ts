...
test("awaiting transfer can be fulfilled with text and uploaded asset ids together", async ({
  request,
}) => {
  const fileContent = "receiver-first mixed payload file";
  const fileSize = Buffer.byteLength(fileContent, "utf8");
  ...
  const fulfillResponse = await request.post(`/api/transfers/${created.code}/payload`, {
    data: {
      payload: {
        text: "receiver mixed payload text",
        uploadedAssetIds: [storedAsset.id],
      },
    },
  });
